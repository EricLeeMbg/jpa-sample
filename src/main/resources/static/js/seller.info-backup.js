...
$(document).ready(function(){
    ...
    $( "#sellerExtraInfo" ).submit(function( event ) {
        ...
        if ( $form.parsley().isValid()) {
            ...
            if($("input[name=CompanyLogoCid]").val() == ""){
            ...
            $.ajax({
                ...
                success: function(result) {
                    if(result.result.resCode == "0000"){
                        ...
                        $.each(formData, function() {
                            var value = this.value;
                            var targetId = $form.find('#'+ this.name);
                            var targetName = $form.find('[data-name='+ this.name +']');
                            
                            // img src 및 파일명
                            if(targetName.data("name") == "CompanyLogoCid"){
                                if(targetName.get(0).tagName == "IMG"){
                                    targetName.attr("data-value", $(this).attr("src"));
                                }
                                value = $form.find(".img-info").text();
                            } else {
                                targetId.attr("data-value", value);
    ...
    $('.crop-popup').click(function(e) {
        ...
        var url = "/crop?maxW=100&maxH=75&formId="+parentId;
